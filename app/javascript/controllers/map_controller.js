...
export default class extends Controller {
  ...
  async connect() {
    ...
    this.markersValue.forEach(({ lat: mLat, lng: mLng, name, city, email, telegram }) => {
      cluster.addLayer(L.marker([mLat, mLng], { icon: userIcon }).bindPopup(this.popupContent(name, city, email, telegram)))
    })
    this.map.addLayer(cluster)
  }

  // Build popup as DOM nodes so user-controlled profile data can't inject HTML (XSS).
  popupContent(name, city, email, telegram) {
    ...
    const { at_symbol: atSymbol, paper_airplane: paperAirplane } = this.iconsValue
    el.appendChild(this.contactDetail(atSymbol, email))
    if (telegram) el.appendChild(this.contactDetail(paperAirplane, telegram))

    return el
  }

  contactDetail(iconUrl, text) {
